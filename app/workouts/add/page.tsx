...
export default function AddWorkoutPage() {
  ...
  const addSet = (exerciseIndex) => {
    ...
    updatedExercises[exerciseIndex].sets.push({
      ...
      rpe: lastSet.rpe, // Mantener el mismo estado de RPE que la última serie
    })
    setExercises(updatedExercises)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Debes iniciar sesión para registrar un entrenamiento",
      })
      return
    }
    ...
    try {
      ...
      // Insert workout with explicit user_id
      ...
      // Insert exercises if there are valid ones
      if (validExercises.length > 0 && workout && workout.length > 0) {
        ...
        for (const exercise of validExercises) {
          ...
          if (validSets.length === 0) {
            ...
            validSets.push({ reps: "0", weight: "0", rpe: 7 })
          }
          ...
          if (exerciseError) {
            console.error("Error al guardar ejercicio:", exerciseError)
            throw exerciseError
          }

          // Guardar cada serie individual en la nueva tabla exercise_sets
          if (exerciseData && exerciseData.length > 0) {
            const exerciseId = exerciseData[0].id

            // Preparar los datos de las series para inserción
            const setsToInsert = validSets.map((set, index) => ({
              exercise_id: exerciseId,
              set_number: index + 1,
              reps: Number.parseInt(set.reps) || 0,
              weight: Number.parseFloat(set.weight) || 0,
              rpe: set.rpe !== null ? set.rpe : null, // Asegurarse de que RPE sea null si no está definido
            }))
            ...
            if (setsError) {
              console.error("Error al guardar series:", setsError)
              // No lanzamos error para que no falle todo el proceso
            }
          }
        }
      }
    ...
    <div className="max-w-2xl mx-auto py-4 px-4 sm:py-8 sm:px-0">
      ...
      <Card className="border-0 shadow-lg bg-gradient-to-br from-card/50 to-card">
        <CardHeader className="pb-4">
          <CardTitle className="text-xl sm:text-2xl font-bold">Nuevo entrenamiento</CardTitle>
          <CardDescription className="text-muted-foreground">
            Registra los detalles de tu sesión de entrenamiento
          </CardDescription>
        </CardHeader>
